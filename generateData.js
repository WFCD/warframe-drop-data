...
const writeData = (data, key, format) => {
  ...
  const bodyStr = JSON.stringify(body, undefined, format);
...
fetch(config.dropDataUrl).then(async (res) => {
  ...
  const totalKeys =
    dataKeys +
    5 + // builds + hash + all + all.slim + info
    missionKeys +
    relicKeys;
  ...
  writeFileSync(resolve(__dirname, 'data', 'builds', 'builds.json'), JSON.stringify(builds, undefined, jsonFormat));

  bar.tick({ file: `/data/builds/${info.hash}.json` });
  writeFileSync(resolve(__dirname, 'data', 'builds', `${info.hash}.json`), JSON.stringify(data, undefined, jsonFormat));

  bar.tick({ file: '/data/all.json' });
  writeFileSync(resolve(__dirname, 'data', 'all.json'), JSON.stringify(data, undefined, jsonFormat));

  bar.tick({ file: 'data/all.slim.json' });
  writeFileSync(resolve(__dirname, 'data', 'all.slim.json'), JSON.stringify(dropSiteData, undefined, jsonFormat));

  bar.tick({ file: 'data/info.json' });
  writeFileSync(resolve(__dirname, 'data', 'info.json'), JSON.stringify(info, undefined, jsonFormat));

  Object.keys(data).forEach((key) => {
    writeData(data, key, jsonFormat);
  });
  ...
  for (const planet of Object.keys(data.missionRewards)) {
    for (const location of Object.keys(data.missionRewards[planet])) {
      ...
      bar.tick({
        file: `/data/missionRewards/${planet}/${location.replace(':', '')}.json`,
      });
      const missionData = { ...data.missionRewards[planet][location] };
      missionData.planet = planet;
      missionData.location = location;
      writeFileSync(
        resolve(__dirname, 'data', 'missionRewards', `${planet}`, `${location.replace(':', '')}.json`),
        JSON.stringify(missionData, undefined, jsonFormat)
      );
    }
  }
  ...
  for (const tier of Object.keys(relicStruct)) {
    for (const relicName of Object.keys(relicStruct[tier])) {
      ...
      writeFileSync(
        resolve(__dirname, 'data', 'relics', `${tier}`, `${relicName}.json`),
        JSON.stringify(relicStruct[tier][relicName], undefined, jsonFormat)
      );
    }
  }
