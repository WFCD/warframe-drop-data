...
async function init(time, first = false) {
  const data = await fetch(`./data/info.json?${time}`).then(d => d.json())
  const hash = data.hash
  const timestamp = data.timestamp

  if (first) console.log("./data/info.json", data)
  const oldDataRaw = localStorage.getItem("_wfinfo") || INFO_DEFAULT
  let oldData
  try {
    oldData = JSON.parse(oldDataRaw)
  } catch (e) {
    // oh no, couldn't parse.... let's override the stored value
    oldData = JSON.parse(INFO_DEFAULT)
  ...
  if (hash !== oldData.hash) {
      ...
      const wfData = await fetch(`./data/all.slim.json?${time}`).then(d => d.body);
      if (isJSON(wfData)) {
        localStorage.setItem("_wfdata", wfData)
        onDataRetrieved(JSON.parse(wfData))
      } else {
        // basically start all over in cases where data isn't present, until it is
        init(time)
      }
  } else {
