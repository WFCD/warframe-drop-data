...
function checkUnique(enemies, enemy) {
    for (let item of enemies) {
        ...
        if (item._id == enemy._id) {
            item._id = hash(`${item.enemyName} ${item.ememyModDropChance}`)
            enemy._id = hash(`${enemy.enemyName} ${enemy.ememyModDropChance}`)

            if (item._id == enemy._id) {
                item._id = hash(`${item.enemyName} ${item.ememyModDropChance} ${item.mods.length}`)
