...
module.exports = {
    ...
    addUniqueHashedObject: function(items, source, properties) {
        for (let item of items) {
            // Always process items of which the base hashes are the same, even after already being processed.
            const baseItemHash = module.exports.hash(item[properties[0]])
            const baseSourceHash = module.exports.hash(source[properties[0]])

            // This will save some time...
            if (item._id === source._id || baseItemHash === baseSourceHash) {
                let itemHash = []
                let sourceHash = []
                ...
                for (let i = 0; i < (properties.length + 1); i++) {
                    if (i < properties.length) {
                        ...
                        if (item._id !== source._id) {
                            // We"ve successfully changed the ID. Bail.
                            break
                        }
                    } else {
