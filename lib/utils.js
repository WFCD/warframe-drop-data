...
export const formatSiteData = (data) => {
  ...
  for (const planetName of Object.keys(data.missionRewards)) {
    ...
    for (let locationName of Object.keys(data.missionRewards[planetName])) {
      ...
      if (Array.isArray(location.rewards)) {
        let rawPlaceName = `${planetName}/${locationName} (<b>${location.gameMode}</b>)`;
        if (rawPlaceName.includes(`(${location.gameMode}) (<b>${location.gameMode}</b>)`)) {
          rawPlaceName = rawPlaceName.replace(` (${location.gameMode})`, '');
        }
  ...
  const bountyKeys = [
    ...
    {
      key: 'entratiLabRewards',
      place: "Deimos/Albrecht's Laboratories",
    },
    {
      key: 'hexRewards',
      place: 'Höllvania',
...
export const parseRotation = (str) => {
  ...
  if (!Array.isArray(res) || res.length !== 2) return undefined;
...
export const parseChance = (str) => {
  ...
  if (!Array.isArray(res) || res.length !== 3) return undefined;
...
export const parseRelic = (str) => {
  const relicRegex = /([A-z]*)\s(?:([A-Z][0-9]+)|([IVXLCDM]*)|(Eterna))\s?Relic\s\(([A-z]*)\)/i;

  const res = str.match(relicRegex);

  if (!Array.isArray(res) || res.length !== 6) return undefined;
  const rn = res[2] || res[3] || res[4] || undefined;
