 ...
 * @param {cheerio.CheerioAPI} $ cheerio interface
...
function operationalSupplyMapper($, element, syndicate, section) {
  ...
  const standing = parseStanding(
    $(element)
      .find('> div:nth-of-type(1) > div:nth-of-type(1) .sortkey')
      .text()
      .replace(/[^\\x00-\x7F]/g, ' ')
      .replace(' Standing', '')
  );
  const credits = parseStanding(
    $(element)
      .find('> div:nth-of-type(1) > div:nth-of-type(2) .sortkey')
      .text()
      .replace(/[^\\x00-\x7F]/g, ' ')
      .replace(' Credits', '')
  );
 ...
 * @property {string} fandomWikiPage The Fandom wiki page name of the syndicate
...
const SYNDICATES = [
  {
    name: 'Steel Meridian',
    fandomWikiPage: 'Steel_Meridian',
    sections: [{ selector: '#mw-customcollapsible-SteelMeridian > div > div' }],
  },
  {
    name: 'Arbiters of Hexis',
    fandomWikiPage: 'Arbiters_of_Hexis',
    sections: [{ selector: '#mw-customcollapsible-ArbitersofHexis > div > div' }],
  },
  {
    name: 'Cephalon Suda',
    fandomWikiPage: 'Cephalon_Suda',
    sections: [{ selector: '#mw-customcollapsible-CephalonSuda > div > div' }],
  },
  {
    name: 'The Perrin Sequence',
    fandomWikiPage: 'The_Perrin_Sequence',
    sections: [{ selector: '#mw-customcollapsible-ThePerrinSequence > div > div' }],
  },
  {
    name: 'Red Veil',
    fandomWikiPage: 'Red_Veil',
    sections: [{ selector: '#mw-customcollapsible-RedVeil > div > div' }],
  },
  {
    name: 'New Loka',
    fandomWikiPage: 'New_Loka',
    sections: [{ selector: '#mw-customcollapsible-NewLoka > div > div' }],
  },
  {
    name: 'Conclave',
    fandomWikiPage: 'Conclave',
    sections: [{ selector: '.flex-container > div' }],
  },
  {
    name: 'Cephalon Simaris',
    fandomWikiPage: 'Cephalon_Simaris',
    sections: [{ selector: '#mw-customcollapsible-Simaris > div > div' }],
  },
  {
    name: 'Ostron',
    fandomWikiPage: 'Ostron',
    sections: [
  ...
  {
    name: 'The Quills',
    fandomWikiPage: 'The_Quills',
    sections: [{ selector: '#mw-customcollapsible-Onkko > div > div' }],
  },
  {
    name: 'Solaris United',
    fandomWikiPage: 'Solaris_United',
    sections: [
  ...
  {
    name: 'Vox Solaris',
    fandomWikiPage: 'Vox_Solaris_(Syndicate)',
    sections: [{ selector: '#mw-customcollapsible-LittleDuck > div > div' }],
  },
  {
    name: 'Ventkids',
    fandomWikiPage: 'Ventkids',
    sections: [{ selector: '#mw-customcollapsible-Ventkids > div > div' }],
  },
  {
    name: 'Entrati',
    fandomWikiPage: 'Entrati',
    sections: [
  ...
  {
    name: 'NecraLoid',
    fandomWikiPage: 'Necraloid',
    sections: [{ title: 'Loid', selector: '#mw-customcollapsible-Loid > div > div' }],
  },
  {
    name: 'The Holdfasts',
    fandomWikiPage: 'The_Holdfasts',
    sections: [
      { title: 'Archimedean', selector: '#mw-customcollapsible-Yonta #gallery-0 > div', mapper: galleryMapper },
      {
        title: 'Archimedean',
        selector: '#mw-customcollapsible-Yonta > p + div, #mw-customcollapsible-Yonta > p + div ~ div',
        mapper: archimedeanMapper,
      },
      {
        title: 'Hombask',
        selector:
  ...
  },
  {
    ...
    fandomWikiPage: "Kahl's_Garrison",
    sections: [{ title: 'Chipper', selector: '#mw-customcollapsible-Chipper > div > div' }],
  },
  {
    name: 'Operational Supply',
    fandomWikiPage: 'Operational_Supply',
    sections: [{ selector: '.flex-container > div', mapper: operationalSupplyMapper }],
  },
];
  ...
  await Promise.all(
    SYNDICATES.map(async (syndicate) => {
      ...
      try {
        ...
        const apiUrl = `https://warframe.fandom.com/api.php?action=parse&page=${syndicate.fandomWikiPage}&prop=text&format=json`;
        data = await fetch(apiUrl).then((res) => res.json()).then((json) => json.parse.text['*']);
      } catch {
